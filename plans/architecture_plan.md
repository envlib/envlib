...
| Field | Type | Derives dataset_version_id | CV Source |
|-------|------|--------------------|-----------|
...
**dataset_version_id**: A deterministic `blake2b(digest_size=12)` hash of these 11 Identity fields, hex-encoded. Computed internally. It strictly enforces uniqueness. Changing any of these fields fundamentally changes what the dataset represents, which requires generating a new dataset rather than updating an existing one.
...
Changing the field order — or any other aspect of this serialization rule (the `None` literal, the `\x1f` delimiter, the UTF-8 encoding, the keyless blake2b construction, the digest size) — would produce different `dataset_version_id`s for the same logical dataset, breaking every existing entry in every catalogue. This rule is stable across implementations and versions and must not change. The order is hash-internal only — user-facing display order (reprs, docs, web UIs) is independent and free to differ.

**dataset_id**: a companion hash computed with the same serialization scheme and field order but with `version` omitted (10 fields). It identifies *the dataset* — the identity that persists across versions — and is stored in the RCG entry and `ds.attrs` alongside `dataset_version_id`. It defines the grouping for the query default "latest version of each matching dataset": among a dataset's versions, latest = greatest `created_at`. **Naming decided 2026-07-07**: envlib gives the word *dataset* to the version-spanning identity (matching tethys's version-less `dataset_id` concept, keeping the term conceptually consistent with cfdb's use of "dataset", and matching how environmental data is spoken about — "the ERA5 dataset", not one frozen revision of it); the per-version combo is named `dataset_version_id` and is the catalogue's primary key. Considered and accepted seam: cfdb's `Dataset` object is per-file and each version is its own cfdb file, so one envlib dataset maps to a small family of cfdb files (one per version) — a storage-boundary detail ("each version of a dataset is stored as its own cfdb file") judged far less confusing than putting version-jargon into the query API users touch daily. Docs convention: say "cfdb file" for the storage artifact; reserve "dataset" for the envlib concept. Known caveat: `created_at` records *first registration*, so back-filling an older version after a newer one makes the back-filled version "latest" — documented, not solved, in v1; pin `version=` explicitly in queries when this matters.
...
**version rules**: a free-form slug (lowercased, `[a-z0-9._-]+`; e.g., `1`, `1.1`, `2026-03`). **The string is the identity** — `1`, `1.0`, and `01` are three different `dataset_version_id`s; envlib does not numerically normalize versions. Pick one spelling convention per series and keep it: re-registering "v1" as `1.0` silently forks the series.

**Field normalization** (applies to Identity and General Metadata):
- **All fields**: whitespace is stripped (`.strip()`) before storage.
- **Free-form user-input Identity fields** (`owner`, `product_code`, `spatial_resolution`, `version`) are additionally lowercased before storage and hashing. This prevents `dataset_version_id` fragmentation from trivial case differences (e.g., `ERA5` vs `era5` vs `Era5`).
- **Slug grammar**: `owner`, `product_code`, and `version` must match ASCII `[a-z0-9._-]+` after lowercasing (`re.fullmatch(r'[a-z0-9._-]+', value)`, no Unicode flags) — no whitespace or control characters — **and must contain at least one letter or digit** (punctuation-only slugs like `.`, `-`, `...` are rejected; identity-core review ruling 2026-07-07 — typo-class garbage must not mint permanent identities). **Non-ASCII input is rejected up front, before lowercasing**, so no locale- or Unicode-dependent case mapping (Turkish `İ` and friends) ever reaches the hash. (The grammar also protects the `\x1f` hash delimiter; `spatial_resolution` has its own stricter grammar above.)
...
**Query normalization**: query kwarg values for queryable fields (Identity + CV fields) are normalized via `.strip().lower()` and matched against the **lowercased** stored value (`stored.lower() == query.lower()`) — lowering only the query side would silently never match CVs whose canonical case is mixed (`license='CC-BY-4.0'` → query `cc-by-4.0` vs stored `CC-BY-4.0`). CVs must therefore contain no case-insensitively colliding entries (none do). This lets users type `cat.query(variable='Temperature', owner='NIWA')` and match stored `temperature` / `niwa`. Case-insensitive querying is a side effect of consistent normalization, not a special flag.

### 2. General Metadata
These fields do not affect identity. `license` and `attribution` are required at registration; the rest are optional.

| Field | Type | Derives dataset_version_id | Status | CV Source |
|-------|------|--------------------|--------|-----------|
...
| derived_from | list[str] \| None | no | optional | list of `dataset_version_id`s and/or DOI URLs |
...
- **derived_from** — list of parent datasets this dataset was computed from (reanalyses, QC'd products, ensembles, etc.). Enables machine-traversable lineage. Each entry is either:
  - a `dataset_version_id` — a 24-character hex string referencing an envlib dataset (in this or another RCG); may reference a dataset not yet registered.
  ...
  Entries are distinguished by format (DOI URLs start with `https://doi.org/`; dataset_version_ids are 24-char hex). Neither form is resolved or validated against any external service at registration time — no network call is made; envlib only checks format. Caveat: `dataset_version_id`, `dataset_id`, and `station_id` all share the same 24-hex format, so a mis-pasted id of the wrong type is format-valid and undetectable — take care that `derived_from` entries are `dataset_version_id`s specifically (lineage means "computed from that specific version").
...
These fields are set automatically by the catalogue on registration. Some are immutable (set once at first insert); some are auto-updated by `cat.register()`. They are stored in the RCG entry but are NOT part of the `dataset_version_id` hash.

| Field | Type | Mutability | Description |
|-------|------|------------|-------------|
| created_at | str (ISO8601 UTC) | immutable | Timestamp of first successful `cat.register()` for this `dataset_version_id`. Used to determine the "latest" version when multiple versions match a query. |
...
- **Self-identification**: the computed `dataset_version_id` and `dataset_id` are also written into `ds.attrs`. On every `cat.register()`, envlib re-derives the hash from the identity attrs and fails on mismatch — catching attrs hand-edited after first registration.
...
**Retraction** (removal of a dataset known to contain incorrect data) is handled in v1 by `cat.deregister(dataset_version_id, delete_data=True)` — deleting the catalogue entry and the underlying cfdb file from its remote. Plain `deregister()` (default `delete_data=False`) merely delists: the entry is removed but the hosted data stays up for existing consumers. Deregistration is also the correction path for a typo'd identity field (identity fields can never be edited in place): deregister, fix the attrs, re-register under the new `dataset_version_id`. There is no tombstone.
...
- **Exactly one primary data variable per dataset.** The primary variable's name in the cfdb file must equal the `Metadata.variable` value (e.g., if `variable='temperature'`, the cfdb file must contain `ds['temperature']`). This makes the primary variable self-identifying — no separate `primary_variable` attribute is needed. Registration via `cat.register()` will fail if `meta.variable` is not present as a data variable in the cfdb file.
- **Ancillary variables** (QC flags, uncertainty estimates, counts, etc.) are permitted alongside the primary variable. They must be declared via the CF `ancillary_variables` attribute on the primary variable. Ancillary variable names are unconstrained (e.g., `temperature_qc`, `temperature_stderr`).
...
- **aggregation_statistic conversion (ruled 2026-07-08)**: tethys `instantaneous` / `continuous` / `sporadic` → `point` (cadence regularity is `frequency_interval`'s concern, not the statistic's); `cumulative` → `sum` — with a migration-time check per series that values are interval sums, not running totals; `mean` → `mean`.
...
- **id values do NOT carry over** — envlib's hashes have different fields and serialization than tethys's ids, for both `dataset_version_id` and `dataset_id` (though envlib's `dataset_id` now matches tethys's version-less `dataset_id` *concept*, per the 2026-07-07 naming decision). `station_id` values DO carry over.
- **Migration code must derive ids via the `Metadata` class**, never the bare `compute_dataset_id`/`compute_dataset_version_id` functions — the class applies the utc_offset reduction rule before hashing; the bare functions trust their inputs verbatim (identity-core review note, 2026-07-07).
...
- **Identity Metadata** is validated at `Metadata` construction (first creation of a dataset). Because Identity fields cannot change for a given `dataset_version_id` (they derive the hash), they are never re-validated on subsequent `cat.register()` calls.
...
- envlib owns the canonical cadence grammar — NOT delegated to pandas offset aliases (whose canonical spellings changed across pandas versions and would destabilize `dataset_version_id`).
...
cat.datasets
# [{'feature': 'atmosphere', 'variable': 'temperature', 'owner': 'niwa', ...},
...
# of each matching dataset (grouped by dataset_id) — "latest" is the greatest
# `created_at` among its versions. An explicit version=... kwarg overrides this and
...
#   2. compute dataset_version_id and extract State Metadata
#   3. write the entry to the local RCG (insert or upsert on dataset_version_id)
...
**RCG keying and entry contents (DONE — ebooklet 0.9.0, released 2026-07-02)**: envlib keys RCG entries directly by `dataset_version_id`, not by the cfdb's internal UUID. This makes uniqueness enforcement a property of the storage layer (can't have two entries with the same `dataset_version_id` key), enables direct lookup (`rcg[dataset_version_id]`) instead of scan-and-filter, and makes S3 object keys human-identifiable. The pinned **entry schema v1** (never-change surface):
...
Key facts envlib builds on: `add(conn, key=dataset_version_id, user_meta=meta)` upserts (same key overwrites, and metadata-only upserts DO push — entries are stamped with write time, not the member's timestamp); keys must match `[A-Za-z0-9._-]+` (hex dataset_version_ids trivially comply); `rcg[key] = conn` and `del rcg[key]` round-trip; `dataset_type`/`crs` are cross-checkable from `remote_meta` for free. Verified end-to-end 2026-07-02 with a registration→query→member-open→metadata-upsert→deregister dry run. **envlib must pin `ebooklet>=0.9.0`.**

**Publish failure handling**: if `cat.publish()` fails after the cfdb push succeeds but before the RCG push completes, the data is on remote S3 but not yet advertised in the remote catalogue. Re-running `cat.publish()` is safe — the cfdb push is idempotent (same data), and the RCG entry write is an upsert on `dataset_version_id`.
...
**Value discovery (added 2026-07-07)**: the browse companion to `query()` — `cat.distinct(field, counts=False)` returns the distinct stored values of any queryable field across the catalogue (sorted list excluding `None`; with `counts=True`, a `{value: count}` dict that includes a `None` key). This answers "what is actually *in* this catalogue" — the vocabularies module answers only "what is *valid*", and cannot help at all for the free-form fields (`owner`, `product_code`, `version`). Plural convenience properties wrap it for the 11 identity fields plus `license` and `dataset_type` (`cat.variables`, `cat.owners`, `cat.features`, `cat.product_codes`, ..., `cat.dataset_types`), so tab-completion self-documents the query vocabulary. Typical browse loop: `cat.variables` → `cat.query(variable=...)` → `ref.open()`.
...
# cf_names = envlib.vocabularies.get_cf_standard_names('temperature', feature='atmosphere')  # -> ['air_temperature']
...
# Validates all requirements, computes dataset_version_id, extracts State Metadata (bbox in
...
- `.dataset_version_id` computed property (blake2b hash of the 11 Identity fields, only available when all 11 are set); `.dataset_id` likewise (version omitted)
...
vocabularies.list('variable')       # -> ['temperature', 'precipitation', 'streamflow', ...]
...
vocabularies.is_valid('variable', 'temperature')  # -> True
...
├── metadata.py              # Metadata class, dataset_version_id hashing, validation
├── vocabularies/
...
2. **Metadata module** — `Metadata` class with CV validation, dataset_version_id hashing, `.to_dict()`
3. **Catalogue module** — `Catalogue` class (RCG-backed, keyed by `dataset_version_id`), `DatasetRef` class with .open(), .datasets, .query(), .refresh(), .register(), .publish(), .validate(), .deregister() with full validation, extent extraction, and upsert logic
...
- `uv run pytest` — unit tests for:
  - **Golden-vector hash tests**: fixed inputs → exact expected hex digests for `dataset_version_id`, `dataset_id`, and `station_id`, committed permanently — these are what actually lock the serialization rules. Must include: an input locking the UTF-8 encoding (bytes that differ across codecs), a station whose coordinate rounds to `-0.0` (locks signed-zero normalization), the explicit little-endian WKB bytes, rejection vectors for non-canonical `spatial_resolution` spellings (`.25deg`, `00.25deg`, `0.250deg`, `1.0km`) and out-of-range `utc_offset`s (`+24:00`, `+13:60`), and the `-00:00` → `+00:00` and offset-reduction (`1h` at `+12:00` → `+00:00`) normalizations
  - dataset_version_id hashing is deterministic and consistent
  ...
  - Latest-version query grouping by `dataset_id`, including the documented back-fill caveat behaviour
